  $(function(){
    ...
    for(var i= 0;i<e.length;i++){
        zong+=(e.eq(i).text())*1* ((e.eq(i).attr("data-val")) *1);
    }
    for(var a= 0;a<m.length;a++){
        zongs+=(m.eq(a).text())*1* ((m.eq(a).attr("data-val")) *1);
    }
    ...
    $(".smoney_c").click(function(){
        ...
        if(on == 'on'){
            ...
            $(this).parent().next().find(".quantity").attr("disabled",true);
...
//            $("#checkIcon-1").attr("","cart-checkbox");
            ...
            $(this).next().attr("checked",true);
              ...
              jia.attr("disabled",false);
               $(this).parent().next().find(".quantity").attr("disabled",false);
              $(this).attr("class","cart-checkbox group-8888 checked smoney_c");
            $(this).attr("data-val","on");
             for(var t =0; t<sms.length;t++){
                if(sms.eq(t).attr("data-val") == "off"){
                    ce = 1;
    ...
    $("#checkIcon-1").click(function(){
        ...
        if($(this).attr("data-val")=="on"){
            ...
            s.eq(a).next().attr("checked",false);
            }
            ...
            for(var a =0;a<s.length;a++){
                ...
                s.eq(a).attr("data-val","on");
                 s.eq(a).next().attr("checked",true);
                var number = s.eq(a).parent().next().find(".quantity").val();
                var obj =  s.eq(a).parent().next().find(".quantity-decrease");
                var zhi = obj.attr("data-val");
                var sm = obj.attr("data-em");
                dianz += sm*1*number;
                gouwuz += zhi*1*number;
            }
            $("#cart_oriPrice").text(dianz);
            $("#cart_rePrice").text(gouwuz);
            $("#emoney_zong").val(dianz);
            $("#smoney_zong").val(gouwuz);
            $("#cart_realPrice").text(gouwuz*1 + dianz*1);
        }
    })
