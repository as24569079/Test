...
//                $("#captcha-container").fadeIn("slow");       
        ...
        $("#yan_sub").click(function(){
            ...
            if(yyy==""){
                ...
                $.post("http://www.ch-d-ch.com/Index/tuwen",{value:yyy},function(res){
                    if(res == 1){
                        alert("验证码错误！请重新输入！");
                    }else{
                        if($.trim($("#telphone").val())!=""&&$.trim($("#telphone").val()).length==11&&$.trim(!isNaN($("#telphone").val()))){
                ...
                $.ajax({
                        type:"post",
                        url:"http://www.ch-d-ch.com/Index/phoneSelect",
                        dataType: "text",
                        data:{user:phone},
                        success:
                        function(data){
                            if(data!=1){
                                $.ajax({
                                    type:"post",
                                    url:"http://www.ch-d-ch.com/Index/verificationCode233",
                                    dataType: "text",
