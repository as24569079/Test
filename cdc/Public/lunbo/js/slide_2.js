(function($){
    $.fn.ckSlide = function(opts){
        ...
        this.each(function(){
            var slidewrap = $(this).find('.banner');//轮播元素父对象
...
//            });
			//点击切换相应序号的图片
            $(this).find('.banner-nav a').each(function(cindex){
            ...
            switch(opts.dir){
                case "x":
                    opts['width'] = $(".banner-wrapper").width();
                    slide.width($(".banner-wrapper").width());
                    slide.height("454px");
                    slidewrap.css({
    ...
    function change(show, hide){
        ...
        if(opts.dir == 'x'){
            ...
            $(this).find('.banner').stop().animate({'margin-top':-y}, function(){opts['isAnimate'] = false;});
            opts['isAnimate'] = true;
        }
		else{
			//默认的淡隐淡出效果
            $(this).find('.banner li').eq(hide).stop().animate({opacity:0});
            $(this).find('.banner li').eq(show).show().css({opacity:0}).stop().animate({opacity:1});
        }
       //切换对应标记的颜色
        $(this).find('.banner-nav a').removeClass('current');
        $(this).find('.banner-nav a').eq(show).addClass('current');
    }
    $.fn.ckSlide.opts = {
